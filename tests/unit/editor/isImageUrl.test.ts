...
describe('isImageUrl — extension before query string', () => {
	it('accepts image URLs with arbitrary query params after the extension', () => {
		expect(isImageUrl('https://cdn.example.com/s/abc/cat.png?dl=0&raw=1')).toBe(true);
	});

	it('accepts image URLs on any host so long as the path ends in an image extension', () => {
		expect(isImageUrl('https://storage.example.com/s/abc/cat.jpg')).toBe(true);
	});
});
