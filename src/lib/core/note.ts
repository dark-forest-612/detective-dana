...
/**
 * Per-note editing lock. `null` when the note is unlocked. Timestamps are
 * unix millis so Firestore security rules can compare them against
 * `request.time.toMillis()` without string/timezone juggling.
 */
...
export interface NoteData {
	...
	/** Collab edit lock. Absent on notes created before Phase 4. */
	lock?: NoteLock | null;
}
