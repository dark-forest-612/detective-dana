<script lang="ts">
	...
	import { consumeAutoEdit } from '$lib/collab/autoEdit.js';
	...
	// the whole save pipeline (repo read + XML serialize) when the incoming
	// doc stringifies identically — catches the type-and-undo case cheaply.
	let lastSavedDocFingerprint: string | null = null;

	// Debounced save — rearmed on every edit, fires SAVE_DEBOUNCE_MS after
	// typing pauses. Heartbeat is a separate 30s cadence in the controller,
	// so idle windows send no content writes.
	...
	// Phase 4 collab lock — same integration pattern as routes/note/[id].
	...
	// Editor stays editable while the lock is `available` so the initial
	// click can land — otherwise TipTap's contenteditable=false swallows
	// the click and `focusin` never fires, leaving us unable to auto-
	// acquire. `handleEditorChange` still gates on `canEdit`, so inputs
	// during the brief pre-acquire window aren't persisted until the lock
	// is actually held.
	const editorEditable = $derived(
		lockState.kind === 'held-by-me' || lockState.kind === 'available'
	);

	// Mirror remote content into TipTap when we're not the live editor.
	$effect(() => {
	...
	/**
	 * Focus-driven lock handling. The desktop workspace makes each note
	 * window's edit state track its focus: whatever note currently owns
	 * focus is the one being edited, and only that one. There is no
	 * explicit edit/exit button — `focusin`/`focusout` bubble up from the
	 * editor (and toolbar/menu descendants), so we listen at the window
	 * root and acquire/release accordingly. Internal focus hops (editor →
	 * toolbar button → editor) stay "within the window" and keep the lock.
	 */
	function handleWindowFocusIn(e: FocusEvent) {
		const ctrl = lockController;
		if (!ctrl) return;
		if (lockState.kind !== 'available') return;
		const from = e.relatedTarget as HTMLElement | null;
		if (from && windowEl?.contains(from)) return;
		void ctrl.acquire();
	}
	...
	function handleWindowFocusOut(e: FocusEvent) {
		const ctrl = lockController;
		if (!ctrl) return;
		if (lockState.kind !== 'held-by-me') return;
		const target = e.relatedTarget as HTMLElement | null;
		if (target && windowEl?.contains(target)) return;
		void ctrl.release();
	}

	const isFavoriteState = $derived(note ? isFavorite(note.guid) : false);
	...
	onMount(() => {
		(async () => {
			...
			lockController = new NoteLockController(guid, flushSave);
			if (consumeAutoEdit(guid)) {
		...
		const unregister = registerFlushHook(guid, () => {
			void flushSave();
		});

		// Keep the notebook select in sync with remote/local notebook changes.
	...
	function handleEditorChange(doc: JSONContent) {
		// Only accept edits when we hold the lock — the editor is
		// non-editable in read-only mode, but auto-link marks can still
		// fire onchange via plugin metadata transactions.
		if (!canEdit) return;
		pendingDoc = doc;
		scheduleSave();
	}

	/** Returns true if a write actually reached the repository. */
	async function flushSave(): Promise<boolean> {
		...
		if (result.ok) {
			...
			// Surface the validation failure; leave the fingerprint so the
			// next flush retries the save once the title is fixed. The
			// TomboyEditor blur-validator plugin keeps the caret pinned in
			// the title block while it's invalid.
	...
	async function handleAction(kind: ActionKind) {
		...
		if (kind === 'toggleFavorite') {
			...
			pushToast(
				added ? '즐겨찾기에 추가되었습니다.' : '즐겨찾기에서 제거되었습니다.'
			);
			return;
		}
...
>
	...
	{#if lockState.kind === 'held-by-other'}
		<div class="lock-banner" role="status">
			🔒 {lockState.holderName ?? '다른 사용자'}님이 편집 중 (읽기 전용)
		</div>
	{/if}
